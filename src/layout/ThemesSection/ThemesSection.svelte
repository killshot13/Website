<script lang="ts">
	...
	const themeColors: Tag[] = [
		{ name: $_("landing.themes.colors.darkGrey"), color: "var(--fds-solid-background-tertiary)" },
		{ name: $_("landing.themes.colors.blueGrey"), color: "hsl(219, 15%, 30%)" },
		{ name: $_("landing.themes.colors.purple"), color: "hsl(261, 43%, 45%)" },
		{ name: $_("landing.themes.colors.yellow"), color: "hsl(43, 100%, 50%)" },
		{ name: $_("landing.themes.colors.coolWhite"), color: "hsl(192, 81%, 14%)" },
		{ name: $_("landing.themes.colors.skyBlue"), color: "hsl(193, 43%, 67%)" }
	];
...
<PageSection class="theme-{currentTheme + 1}" id="themes-section">
	...
	<div class="themes-section-content">
		<HeaderChip>{$_("landing.themes.chip")}</HeaderChip>
		<TextBlock variant="titleLarge">{$_("landing.themes.title")}</TextBlock>
		<p>{$_("landing.themes.description")}</p>
		<div class="theme-chooser">
			{#each themeColors as color, i}
				<ColorSwatch
					...
					value={i}
					colorName={color}
					aria-label={$_("landing.themes.colorSwatch", { values: { num: i + 1 }})}
				/>
			{/each}
		</div>
	</div>
